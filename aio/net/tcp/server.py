...
class TaskGuard:
    ...
    async def __aexit__(
        ...
        done, pending = await asyncio.wait(self._tasks, timeout=self._graceful_shutdown)
        for task in done:
    ...
    def create_task(self, coro: Coroutine[Any, Any, None], name: Optional[str] = None) -> asyncio.Task[None]:
